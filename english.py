...
# Set font and handle minus signs for plots
plt.rcParams['font.family'] = 'Malgun Gothic'
plt.rcParams['axes.unicode_minus'] = False

# Encode English alphabet
alphabet = list('abcdefghijklmnopqrstuvwxyz')
char_to_index = {char: i for i, char in enumerate(alphabet)}
index_to_char = {i: char for i, char in enumerate(alphabet)}

# Generate training data (random pairs of letters)
...
# Initialize variables to store loss and accuracy over sessions
history_loss = []
history_accuracy = []
...
# Simulate learning sessions until the target accuracy is reached
def simulate_learning_sessions(target_accuracy=0.95):
    ...
    while True:
        ...
        accuracy = correct / len(test_data)
        print(f"세션 수 {session}: 정확도 = {accuracy * 100:.2f}%")
